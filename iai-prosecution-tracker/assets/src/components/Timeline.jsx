...
import {
	classify,
	computeEntityTimeline,
	getEntityRateAtDate,
} from '../utils/feeClassifier';
...
function Timeline( { application, transactions, loading } ) {
	...
	const timelineData = useMemo( () => {
		...
		const validEvents = classifiedEvents.filter(
			( e ) => e.classification
		);
	...
	const filteredEvents = useMemo( () => {
		...
		return timelineData.events.filter( ( event ) => {
			...
			if ( category === 'milestone' ) {
				return filters.milestones;
			}
			if ( category === 'rce' ) {
				return filters.rce;
			}
			if ( category === 'appeal' ) {
				return filters.appeals;
			}
	...
	const yearMarkers = useMemo( () => {
		...
		for ( let year = startYear; year <= endYear; year++ ) {
			...
			const totalDuration =
				timelineLayout.endDate - timelineLayout.startDate;
			const x =
				( elapsed / totalDuration ) * ( timelineLayout.width - 100 ) +
				50;
	 ...
	 * Handle event click
	 *
	 * @param {Object} event - Event object containing date, code, and classification.
	 */
	 ...
	 *
	 * @param {string} filterKey - Key of the filter to toggle.
	 */
		...
		return (
			<div className="iai-pt-timeline">
				<div className="iai-pt-timeline__error">
					<AlertCircle
						size={ 48 }
						className="iai-pt-timeline__error-icon"
					/>
	...
	return (
		<div className="iai-pt-timeline">
			...
			<div className="iai-pt-timeline__header">
				...
				<div className="iai-pt-timeline__app-meta">
					<div className="iai-pt-timeline__meta-item">
						<Calendar size={ 16 } />
						<span className="iai-pt-timeline__meta-label">
							Filed:
						</span>
					...
					<div className="iai-pt-timeline__meta-item">
						...
						<span>
							{ application?.applicationStatusDescriptionText ||
								'Unknown' }
						</span>
					</div>
				</div>
			...
			<div className="iai-pt-timeline__filters">
				<div className="iai-pt-timeline__filters-label">
					Filter Events:
				</div>
				<div className="iai-pt-timeline__filter-group">
					<label className="iai-pt-timeline__filter-item">
						...
						/>
						<span className="iai-pt-timeline__filter-label">
							Fees
						</span>
					</label>
					<label className="iai-pt-timeline__filter-item">
			...
			<div className="iai-pt-timeline__content">
				...
				{ /* SVG Timeline */ }
				<div
					className="iai-pt-timeline__svg-wrapper"
					ref={ timelineRef }
				>
					<svg
				...
				<div className="iai-pt-timeline__legend">
					...
					<div className="iai-pt-timeline__legend-items">
						<div className="iai-pt-timeline__legend-item">
							<div
								className="iai-pt-timeline__legend-icon"
								style={ { color: '#2563EB' } }
							>
								{ ( () => {
									const StoreIcon =
										getIconComponent( 'store' );
